...
def user_details():
    ...
    if st.button("Login"):
        st.write(f"{username} has looged in.")
...
def main():
    ...
    st.metric("Temperature in Degrees Celcius", 26, 2)

    st.metric("Rainfall in mm", 1400, -350)

    if st.button("Submit"):
        st.write("The button had been pressed")
    ...
    if terms_conditions:
        ...
        gender = st.radio("Selelct your gender", ("Male", "Female", "Other" ))
        if gender == "Male":
            st.write("The men`s conference will be held on Saturday")
        elif gender == "Female":
            st.write("The girl`s conference will be held on Friday")

    teams = st.selectbox("Select your favourite team :" , ["Liverpool", "Arsenal", "Nottingham", "Chelsea"])
    breakfast = st.multiselect("What did you have for breakfast?",["Eggs", "Bacon", "Coffee", "Tea", "Milk", "Cereals", "Sausages"])
    ...
    data_set = st.date_input("Enter the appointment date here : ")

    st.time_input("Enter the time you want to schedule the appointment : ")

    name = st.text_input("Enter your first name here  : ")
    name = st.text_input("Enter your second name here  : ")
    st.text_area("Write an essay why you should be considered for scholarship opportunity", height = 180)
    ...
    #     time.sleep(5)
    #     st.write("The content has been loaded")
    #
    # # progress bar
    # for i in range(100):
    #     st.progress(i)
